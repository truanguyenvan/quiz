...
router.post('/createQuiz', async (req, res, next) => {
  ...
  quiz
    .save()
    .then(async (result) => {
      console.log('Create Quiz Success: ', result.quizName);
      await res.status(200).json({ code: '0', data: result, message: 'ok' });
    })
    .catch((err) => {
      console.log('Create Quiz Failed: ', err.message);
      res.status(400).json({
        code: '10',
        data: null,
        message: err._message ? err._message : 'System Error',
      });
    });
});
...
router.delete('/delete/:quizId', async (req, res, next) => {
  let quizId;
  try {
    quizId = mongoose.Types.ObjectId.createFromHexString(req.params.quizId);
  } catch (err) {
    return res.status(400).json({
      code: '10',
      data: null,
      message: err._message ? err._message : 'Body invalid',
    });
  }
  await Question.deleteMany({ quizId: quizId })
    .then(async () => {
      await Quiz.deleteOne({ _id: quizId })
        .then(() => {
          res.status(200).json({ code: '0', data: null, message: 'ok' });
        })
        .catch((err) => {
          console.log('Delete Quiz Failed: ', err.message);
          return res.status(400).json({
            code: '10',
            data: null,
            message: err._message ? err._message : 'System Error',
    ...
    .catch((err) => {
      console.log('Delete Question Failed: ', err.message);
      res.status(400).json({
        code: '10',
        data: null,
        message: err._message ? err._message : 'System Error',
...
///Update Quiz
router.patch('/updateDetails/:quizId', async (req, res, next) => {
  let quizId;
  try {
    quizId = mongoose.Types.ObjectId.createFromHexString(req.params.quizId);
  } catch (err) {
  ...
  await Quiz.findById(quizId)
    ...
    .then(async (result1) => {
      ...
      Quiz.findOneAndUpdate(
        ...
        .exec()
        .then((result) => {
          res.status(200).json({ code: '0', data: result, message: 'ok' });
        })
        .catch((err) => {
          console.log('Update Detail Quiz Failed: ', err.message);
          res.status(400).json({
            code: '10',
            data: null,
            message: err._message ? err._message : 'System Error',
